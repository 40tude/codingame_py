...
def BuildParseTree(Expression, Dic):
  ...
  for i in Expression:
    if i in ['(', '[']:
      ...
      if i==']':
        # Si il y a des childs, faire la somme // de tous les Req des childs
        # sinon faire la somme // de tous les operands
        tmp = 0.0
        if (len(CurrentTree.Childs)):
          for child in CurrentTree.Childs:
            tmp = tmp + 1/child.Req
        else:
